 ...
 * @Version1.0
 */
public class TrxOrderSqlProvider {

    //所有的列名
    private String columns="id, order_no, user_id, activity_id, open_id, transaction_id,prepay_id,prepayid_duetime, from_type,create_ip, type, price, number, amount, fee_type, bank_type, create_time, " +
            "finish_time, transaction_status, order_status, trade_type,code_url,sys_type,buyer,buyer_phone,buyer_email, version,QRCodeTicket_url,over_time";
    private String columnsAlis="trx.id, trx.order_no, trx.user_id, trx.activity_id, trx.open_id, trx.transaction_id, trx.prepay_id, trx.prepayid_duetime, trx.from_type, trx.create_ip, trx.type, trx.price, " +
            "trx.number, trx.amount, trx.fee_type, trx.bank_type, trx.create_time, trx.finish_time, trx.transaction_status, trx.order_status, trx.trade_type, trx.code_url, trx.sys_type,trx.buyer,trx.buyer_phone,trx.buyer_email,trx.version,trx.QRCodeTicket_url,trx.over_time";
    //保存
        ...
        return new SQL(){
            {
                ...
                if(trxOrder.getActivityId()!=null){
                ...
                if(trxOrder.getSysType()!=null){
                    VALUES("sys_type","#{sysType}");
                }
                if(trxOrder.getBuyer()!=null){
                    VALUES("buyer","#{buyer}");
                }
                if(trxOrder.getBuyerPhone()!=null){
                    VALUES("buyer_phone","#{buyerPhone}");
                }
                if(trxOrder.getBuyerEmail()!=null){
                    VALUES("buyer_email","#{buyerEmail}");
                }
                if(trxOrder.getQRCodeTicketUrl()!=null){
                    VALUES("QRCodeTicket_url","#{QRCodeTicketUrl}");
                }
                if(trxOrder.getOverTime()!=null){
                    VALUES("over_time","#{overTime}");
                }
            }
        }.toString();
    }
        ...
        return new SQL(){
            {
                ...
                if(param.get("activityId")!=null&&param.get("activityId")!=""){
                    WHERE("trx.activity_id=#{activityId}");
                }
                ...
                if(param.get("searchKeys")!=null&&param.get("searchKeys")!=""){
                    WHERE("(trx.order_no like concat(#{searchKeys},'%') or trx.buyer like concat(#{searchKeys},'%') or trx.buyer_phone like concat(#{searchKeys},'%') or trx.buyer_email like concat(#{searchKeys},'%'))");
                }
                ...
                if(param.get("sysType")!=null&&param.get("sysType")!=""){
                    WHERE("trx.sys_type=#{sysType}");
                }
                if(param.get("buyer")!=null&&param.get("buyer")!=""){
                    WHERE("trx.buyer=#{buyer}");
                }
                if(param.get("buyerPhone")!=null&&param.get("buyerPhone")!=""){
                    WHERE("trx.buyer_phone=#{buyerPhone}");
                }
                if(param.get("buyerEmail")!=null&&param.get("buyerEmail")!=""){
                    WHERE("trx.buyer_email=#{buyerEmail}");
                }
