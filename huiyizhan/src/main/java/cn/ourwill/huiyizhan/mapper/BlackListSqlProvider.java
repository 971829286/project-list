...
public class BlackListSqlProvider {
    ...
    public String selectByParam(final Map<String,Object> param){
        return new SQL(){
            {
                SELECT("b.id,b.user_id,(case when b.status = 0 then 0 " +
                        "     when status = 2 then 2 " +
                        "     when (status = 1 and now()> start_date and now()< end_date) then 1" +
                        "     else 0 end) as status,b.start_date,b.end_date,b.reason,b.c_id,b.c_time,b.u_id,b.u_time,u.nickname,u.username");
                FROM("black_list b ");
