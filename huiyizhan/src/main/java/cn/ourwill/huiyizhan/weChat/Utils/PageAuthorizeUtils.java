...
import cn.ourwill.huiyizhan.utils.HttpClinetUtils;
...
public class PageAuthorizeUtils {
    ...
    public static UserTokenReturn getUserAccessToken(String code) throws Exception {
        //appid:公众号的唯一标识
    ...
    public static UserInfoReturn getUserInfo(String access_token,String openid) throws Exception{
...
//            String reObj = HttpURlUtils.getUrlResponse(accessUrl);
            Map map = HttpClinetUtils.sendGet(accessUrl, Map.class);
            if (null != map) {
                try {
...
//        JSONObject jsonObject = httpsRequest(requestUrl, "GET", null);
    ...
    // 微信扫一扫
    public static String getQRCodeTicket(QRCodeParam QRCodeParam) throws Exception{

        // 第一步，生成带参数的二维码。
        String url="https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token=ACCESS_TOKEN";
        String requestUrl = url.replace("ACCESS_TOKEN", QRCodeParam.getAccess_token());
        JSONObject jsonObject = new JSONObject();
        JSONObject scene = new JSONObject();
        JSONObject action_info = new JSONObject();
        scene.put("scene_str",QRCodeParam.getScene_str());
        action_info.put("scene",scene);
        jsonObject.put("action_name","QR_STR_SCENE");
        jsonObject.put("expire_seconds",QRCodeParam.getExpire_seconds());
        jsonObject.put("action_info",action_info);

        String returnStr=HttpURlUtils.httpsRequest(requestUrl,"POST",jsonObject.toString());
        log.info("getQRCodeTicket.returnStr"+returnStr);
        if(returnStr.indexOf("errcode")<0) {
            ObjectMapper mapper = new ObjectMapper();
            QRCodeTicket ticket = mapper.readValue(returnStr, QRCodeTicket.class);
            //第二步，根据二维码Ticket获取二维码图片url。
            String ticketUrl="https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=TICKET";
            String requesTickettUrl = ticketUrl.replace("TICKET", ticket.getTicket());
            return requesTickettUrl;
        }
        return null;
    }
}
