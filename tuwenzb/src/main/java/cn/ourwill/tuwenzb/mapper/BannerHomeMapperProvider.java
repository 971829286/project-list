...
public class BannerHomeMapperProvider {
		...
		return new SQL(){
			{
				...
				if(bannerHome.getPhotoLive()!=null){
	...
	public String findAll(){
		return new SQL(){
			{
				SELECT("id,priority,pic,title,link,c_time,c_user,u_time,u_user,photo_live");
				FROM("banner_home");
	...
	public String selectByParmer(final Map<String,Object> param){
		return new SQL(){
			{
				SELECT("id,pic,title,link,c_time,c_user,u_time,u_user,photo_live");
				FROM("banner_home");
