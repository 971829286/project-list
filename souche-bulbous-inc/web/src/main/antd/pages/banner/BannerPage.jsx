...
import {Form, Table, Radio, Button, Pagination, Popconfirm, Input, Modal, Icon, Upload, Select, Divider} from 'antd';
...
class BannerPage extends SADPage {
    ...
    handleOk = () => {
        ...
        if(typeof value.title == 'undefined'|| value.title == null || value.title == ''){
            ...
            let str = this.state.fileList[0].response.data || this.state.fileList[0].url;
            let splits = str.split('/');
            let url = "/"+splits[3] + "/" + splits[4] +"/"+splits[5];
            this.state.itemData.image = url;
            //如果是新增
                    ...
                    return (
                        <span>
                            {/*<Popconfirm title={this.displayOperators(record.status)}*/}
                                        ...
                                        {/*onConfirm={() => this.onChangeStatus(record.id, this.getOperator(record.status))}>*/}
