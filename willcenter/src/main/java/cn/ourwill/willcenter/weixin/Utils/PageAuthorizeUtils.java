...
import cn.ourwill.willcenter.weixin.pojo.*;
...
public class PageAuthorizeUtils {
    ...
    public static void getCode(HttpServletRequest request, HttpServletResponse response)throws Exception{
        String redirect_uri="http://live.tuwenzhibo.com/dev/code";
    ...
    //拉取用户信息
    public static UserInfoReturn getUserInfo(TokenReturn tokenReturn) throws Exception{
        //access_token:网页授权接口调用凭证,注意：此access_token与基础支持的access_token不同
        String access_token=tokenReturn.getAccess_token();
        //openid:用户的唯一标识
        String openid=tokenReturn.getOpenid();
        log.info("getUserInfo.openid"+openid);
        //lang:返回国家地区语言版本，zh_CN 简体，zh_TW 繁体，en 英语
        String getURL="https://api.weixin.qq.com/sns/userinfo?access_token="+access_token+"&openid="+openid;
        String returnStr=HttpURlUtils.getUrlResponse(getURL);
        log.info("getUserInfo.returnStr"+returnStr);
        if(returnStr.indexOf("errcode")<0) {
            ObjectMapper mapper = new ObjectMapper();
