...
public class MaintenanceQuerity {
    ...
    public String wbNotify(HttpServletRequest request) {
        ...
        try {
            ...
            if (StringUtils.equals(data.getString("status"), "FAIL")) {
                //设置为失败的结果
                //退款
                boolean refund = maintenanceOrderManager.refund(orderId);
                //更改退款状态
                int refundStatus = 2;
                if (refund) {
                    refundStatus = 1;
                }
                MaintenanceRecordDO maintenanceRecordDO = new MaintenanceRecordDO(orderId, 5, "2", reason, serviceNo, null);
    ...
    public Result<Map<String, Object>> queryCarByVin(@ApiParam(value = "token", required = true)
        ...
        //创建订单
        OrderDTO order = maintenanceOrderManager.createOrder(vin, coupon_codes, engine, license_plate);
        Map<String, Object> resultMap = new HashMap<>();
        //返回参数
    ...
    public Result<PaymentDTO> preparePay(@ApiParam(value = "order_code", required = true) @Param(value = "order_code", required = true) String order_code,
                                         @ApiParam(value = "amount", required = true) @Param(value = "amount", required = true) Double amount,
        ...
        logger.info("获取预支付信息 amount={}, orderCode={}, payer_id={}, payer_type={}", amount, order_code, payer_id, payer_type);
        Result<PaymentDTO> result = new Result<>();
        BigDecimal bigDecimal1 = new BigDecimal(amount.toString());
        BigDecimal multiply = bigDecimal1.multiply(new BigDecimal(100).setScale(0, BigDecimal.ROUND_DOWN));
        PaymentDTO paymentDTO = orderCenterSPI.preparePay(order_code, multiply.longValue(), payer_id, payer_type);
    ...
    public Result<Map<String, Object>> payCoupons(@ApiParam(value = "token", required = true)
                                                  @Param(value = "token", required = true) String token) {
        Result<Map<String, Object>> result = new Result<>();
        List<CouponBaseVO> couponListDTOSResult = null;
        logger.info("用户卷列表");
        try {
            List<CouponListDTO> couponListDTOS = orderCenterSPI.userCoupons();
            //筛除和业务不相关的优惠券
            couponListDTOSResult = new ArrayList<>();
            for (CouponListDTO couponListDTO : couponListDTOS) {
                String bizCode = couponListDTO.getBizCode();
                if (bizCode.equals(Constants.BUSSIZE_TYPE)) {
            ...
            logger.info("用户卷列表 异常 e={}",e.toString());
        }
        Map<String, Object> resultMap = new HashMap<>();
