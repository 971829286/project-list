...
import com.souche.optimus.common.util.*;
...
/**
 * @program: niu
 * @ClassName: WeiBaoMsgPushSPIAdapter
...
@Component
public class WeiBaoMsgPushSPIAdapter implements WeiBaoMsgPushSPI {

    private static final Logger LOGGER = LoggerFactory.getLogger(WeiBaoMsgPushSPIAdapter.class);
    ...
    public void MsgPush(String msgPushContent, String orderId) {
        ...
        try {
            if ("success".equals(msgPushContent.toLowerCase())) {
                ...
                bodyText = "您的保养记录查询失败，查询费用退换至支付用户，具体失败原因点击进入APP查看";
            ...
            String result = HttpClientPostUtil.postUrl(msgPushVO.getMsgPushUrl(), params, headers);
            ...
            LOGGER.error("维保推送消息异常 exception={}", e);
        }
    }
    ...
    public String getUserId(String orderId) {
        ...
        try {
            ...
            UserObject user = userAPI.findOne(params, Constants.USERAPI_FINDONE_APPNAME);
            LOGGER.info("查询userId的结果 userId={}", user.getId());
            return user.getId();
        } catch (Exception e) {
            LOGGER.error("查询用户Id异常 exception={}", e);
            return null;
        }
    }
